...
layui.use(['table', 'jquery', 'myutil', 'ht_config'], function () {
    ...
    function deleteModel(obj, modelId, deploymentId) {
        ...
        layer.confirm('是否删除？', function (index) {
            ...
            $.ajax({
                cache: true,
                type: "GET",
                url: basePath + 'deleteModel?modelId=' + modelId,
    ...
    function exportModel(modelId) {
        window.open(basePath + '/export/' + modelId);
    ...
    function deploy(modelId) {
        layer.confirm('您确定发布测试版吗？', function (index) {
            ...
            $.ajax({
                cache: true,
                type: "GET",
                url: basePath + 'deploy?modelId=' + modelId,
                async: false,
                timeout: 10000,
                error: function (request) {
                    layer.msg("发布失败！");
    ...
    function startProc(modelProcdefId, modelVersion) {
        layer.confirm('您确定启动流程吗？', function (index) {
            ...
            $.ajax({
                cache: true,
                type: "GET",
                url: basePath + 'start?procDefId=' + modelProcdefId + "&version=" + modelVersion,
                async: false,
                timeout: 10000,
                error: function (request) {
                    layer.msg("启动失败！");
